...
@Component({
  selector: 'app-treasure-dividend-one',
  templateUrl: './treasure-dividend-one.component.html',
  styleUrls: ['./treasure-dividend-one.component.css']
})
export class TreasureDividendOneComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    setTimeout(async () => {
      ...
      const values = await this.treasureManager.getHoldingDividends(today, 7);

      this.dataArray = values.map((v, i) => {
        var d = new Date((today - i - 1) * 1000 * 24 * 3600);
